...
def death_screen():
    ...
    # Render and display high scores
    high_scores = get_high_scores()
    y_position = 400
    for name, scores, time in high_scores:
        previous_score_text = FONT.render(f"{name}: Score: {scores}, Time: {time}", True, BLACK)
        screen.blit(previous_score_text, (400, y_position))
        y_position += 35

    screen.blit(death_text, (500, 100))
    screen.blit(previous_score_text, (500, 200))
    screen.blit(input_text, (400, 300))
...
#Controls
    ...
    if player.lives == 0:
        ...
        screen.blit(score_text, (500, 150))
